...
function analyzeManagementQuality(financialLineItems, companyNews) {
  ...
  const capexValues = financialLineItems
    .filter(
      (fi) =>
        fi.capital_expenditure !== null && fi.capital_expenditure !== undefined
    )
    .map((fi) => fi.capital_expenditure);
...
function analyzeInnovation(financialLineItems) {
  ...
  const rdValues = financialLineItems
    .filter(
      (fi) =>
        fi.research_and_development !== null &&
        fi.research_and_development !== undefined
    )
    .map((fi) => fi.research_and_development);
  ...
  const smValues = financialLineItems
    .filter(
      (fi) =>
        fi.sales_and_marketing !== null && fi.sales_and_marketing !== undefined
    )
    .map((fi) => fi.sales_and_marketing);
